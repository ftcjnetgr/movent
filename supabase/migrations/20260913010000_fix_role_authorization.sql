...
create function public.current_user_role() returns text language sql stable security invoker set search_path=public as $$ select p.role from public.profiles p where p.id=auth.uid() and p.status='Active' and p.is_locked=false limit 1 $$;
create function public.is_admin_or_dispatcher() returns boolean language sql stable security invoker set search_path=public as $$ select public.current_user_role() in ('Dispatcher','Controller','Super User') $$;
create function public.is_executor() returns boolean language sql stable security invoker set search_path=public as $$ select public.current_user_role()='Executor' $$;

create or replace function public.transition_assignment(p_assignment_id uuid,p_next_status text,p_actor_id uuid default auth.uid()) returns public.assignments language plpgsql security invoker set search_path=public as $$
...
begin
 ...
 elsif v_role not in ('Dispatcher','Controller','Super User') then raise exception 'Tidak berwenang mengubah status assignment'; end if;
...
revoke execute on function public.normalize_schedule_import() from public;
...
create policy assignments_update_operational on public.assignments for update to authenticated using (public.is_admin_or_dispatcher() or exists(select 1 from public.profiles p where p.id=auth.uid() and p.nik=assignments.executor_nik and p.role='Executor' and p.status='Active' and p.is_locked=false)) with check (public.is_admin_or_dispatcher() or exists(select 1 from public.profiles p where p.id=auth.uid() and p.nik=assignments.executor_nik and p.role='Executor' and p.status='Active' and p.is_locked=false));
create policy assignment_events_insert_operational on public.assignment_events for insert to authenticated with check (public.is_admin_or_dispatcher() or exists(select 1 from public.assignments a join public.profiles p on p.nik=a.executor_nik where a.id=assignment_events.assignment_id and p.id=auth.uid() and p.role='Executor' and p.status='Active' and p.is_locked=false));
